...
Colate_claude_solar <- function(target){
  ...
  while (!is.na(file_path)){
    ...
    if (length(is_year) == 0 | sum(is_year) == 0){
      write_lines(file_path, paste0(output_dir, "missing_year_needs_investigating.txt"),
                  sep = "\n", append = TRUE)
      file_path <- next_file(output_dir)
      next
    }
...
# value that describes the type of data to expect on that row. For claude, 239 
