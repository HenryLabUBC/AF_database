...
get_cassiope_solar <- function(){
  ...
  # pre-2008, before hourly data was also collected
   if (!file.exists("/media/ross/external/Copying_files/archive/cassiope_solar/done.txt")){
   target = "(^|,) *\"?107\"? *(,|$)"
   Colate_cassiope_solar(target)
   write_lines("finished 107", "/media/ross/external/Copying_files/archive/cassiope_solar/done.txt")
   }
  # A REGEX expression describing the desired identifier as the sole data in a 
  #spreadsheet cell, with any amount of white space permitted, as well as optional
...
Colate_cassiope_solar <- function(target){
  ...
  output_dir = "/media/ross/external/Copying_files/archive/cassiope_solar/"
  ...
  while (!is.na(file_path)){
    ...
    if (!grepl("cass", file_path, ignore.case = TRUE)){
      file_path <- next_file(output_dir)
      next}
...
save_solar <- function (solar_dataframe, output_dir, is_year){
  ...
  if (first_year != last_year){
    file_name <- paste0(output_dir, "/cassiope_solar_",as.character(first_year),
