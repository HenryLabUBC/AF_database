...
get_meadow_solar <- function(){
  ...
  # if (!file.exists("/media/ross/external/Copying_files/archive/meadow_solar/done.txt")){
  # target = "(^|,) *\"?107\"? *(,|$)"
  # Colate_meadow_solar(target)
  # write_lines("finished 107", "/media/ross/external/Copying_files/archive/meadow_solar/done.txt")
  # }
  # A REGEX expression describing the desired identifier as the sole data in a 
  #spreadsheet cell, with any amount of white space permitted, as well as optional
...
Colate_meadow_solar <- function(target){
  ...
  output_dir = "/media/ross/external/Copying_files/archive/meadow_solar/"
  ...
  while (!is.na(file_path)){
    ...
    if (grepl("cass|hobo|tower|PMB|bay|fosh|clau", file_path, ignore.case = TRUE)){
      file_path <- next_file(output_dir)
      next}
...
solar_identifier <- function(dataframe, file_path, output_dir){
  ...
  for (i in 1:(length(dataframe) - 1)){
    ...
    if (is.na(squeezed_range[2]) |
        ...
        (squeezed_range[1] & squeezed_range[5] == 0)) {
      is_solar <- append(is_solar, FALSE)
      next}
