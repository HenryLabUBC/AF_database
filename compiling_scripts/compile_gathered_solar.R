...
compile_solar <- function(site, site_abreviation, input_dir, output_dir, W_or_KW, fix_cols = FALSE){
  ...
  # If site is meadow then override the elimination of duplicated data. The duplicates have been manually
  # checked and are the result of mixing max and mean data. Take the lower values to correspond to mean data
  rm.conflicted_duplicates <- TRUE
  if (grepl("mead|fosh", site, ignore.case = TRUE)){
    rm.conflicted_duplicates <- FALSE
  }
  # Remove outliers and duplicates; create file containing any disagreeing data
  compiled_clean_data <- clean_data(compiled_solar,
                                    upper_limit,
                                    lower_limit,
                                    "solar",
                                    output_dir,
                                    rm.conflicted_duplicates
  )
  # remove all but the first solar column. Leaving it in will require a much improved function to
  # determine difference between mean and max data.
  compiled_clean_data <- compiled_clean_data %>% select(!matches("solar_[0-9]"))
  ...
  if (sum(is_hour) > 0){
    ...
    daily_data_simplified <- daily_data %>% select(year, day, 
                                                   matches("cloudy_solar_(mean|max)_(W|KW)"),
                                                   fractional_year,
                                                   calc_max_solar,
                                                   calc_mean_solar,
                                                   calc_mean_adjusted)
    ...
    compiled_processed_data <- compiled_processed_data %>%
      ...
      rename("solar_mean_{tolower(W_or_KW)}" := solar, calc_mean_adjusted = calc_mean_adjusted)
    names(compiled_processed_data) <- tolower(names(compiled_processed_data))
    names(daily_data) <- tolower(names(daily_data))
    # Give all data a plot_id, standardise column names and site names
    compiled_processed_data <- standardise_plots(compiled_processed_data, output_dir, "")
    ...
    compiled_processed_data <- process_data(compiled_clean_data, site_abreviation, W_or_KW, output_dir, fix_cols)
...
make_plots <- function(compiled_processed_data, output_dir, site, summary = TRUE){
  ...
  for (name in solar_names){
    ...
    for (current_year in years){
      ...
      year_data_sunny <- filter(sunny, year == current_year)
      year_data_cloudy <- filter(cloudy, year == current_year)
      year_data_v.cloudy <- filter(v.cloudy, year == current_year)
      year_data_snow <- filter(snow, year == current_year)
      year_data <- filter(compiled_processed_data, year == current_year)
      ...
      jpeg(paste0(output_dir, "/",name, "/",
                  current_year, site, name, ".jpeg"), width = 1400, height = 1400)
      # op <- par(mfcol = c(2,1))
...
gather_data <- function(input_dir){
  ...
  gathered_data <- hablar::convert(gathered_data, hablar::chr(file),
                                   ...
                                   hablar::lgl(dplyr::any_of(matches("converted"))))
...
split_mean_and_max <- function(processed_solar, W_or_KW, output_dir, fix_cols = FALSE){
  ...
  for (name in solar_names){
    ...
    for (current_year in unique(processed_solar$year)){
      ...
      if ((r2_max < 0.25 & r2_ave < 0.25)| abs(r2_max - r2_ave) < 0.025){
        unknown_list <- append(unknown_list, list(year_solar))
        unknown_years <- append(unknown_years, current_year)
      } else if (r2_max >= r2_ave){
    ...
    if (length(unknown_years) > 0){
      if (length(mean_years) > length(max_years)){
        unknown_solar <- do.call(rbind, unknown_list)
        unknown_solar <- unknown_solar %>% select(!starts_with("solar")) %>%
          rename("solar_mean_{W_or_KW}" := current_solar)
        solar_ave_list <- append(solar_ave_list, list(unknown_solar))
        mean_years <- append(mean_years, unknown_years)
      } else {
        unknown_solar <- do.call(rbind, unknown_list)
        unknown_solar <- unknown_solar %>% select(!starts_with("solar")) %>%
          rename("solar_max_{W_or_KW}" := current_solar)
        solar_max_list <- append(solar_max_list, list(unknown_solar))
        max_years <- append(max_years, unknown_years)
      }
    }
  ...
  for (name in solar_names){
    ...
    for (current_year in unique(processed_solar$year)){
      ...
      if (grepl("mean", name)){
        ...
        max_list <- append(max_list, list(year_solar))
      }
    }
  }
...
detect_snow <- function(processed_solar){
  ...
  for (name in solar_names){
    ...
    for (selected_year in years){
      processed_spring <- processed_solar %>% filter(year == selected_year, day > 70 & day < 140)
      early_spring <- processed_solar %>% filter(year == selected_year, day > 70 & day < 110)
      
      # If there are no non-NA data present in the early spring, skip to the next year
      if (length(early_spring[[solar_index]][!is.na(early_spring[[solar_index]])]) < 12|
          length(early_spring[[roll_index]][!is.na(early_spring[[roll_index]])]) < 12) {next}
      ...
      if ((length(early_spring[[cloud_index]][early_spring[[cloud_index]] == "v.cloudy"]) +
        ...
        # snow_melt_day
        processed_solar[[cloud_index]] <- if_else(processed_solar$year == selected_year &
...
clean_data <- function(dataframe, upper_limit, lower_limit, name_pattern, output_dir, rm.conflicted_duplicates = TRUE){
  # Get names of all columns to clean
  names <- grep(name_pattern, names(dataframe), ignore.case = TRUE, value = TRUE)
  # If no column exists to say that data has been converted from a different unit then
  # add one filled with FALSE
  if (!"converted" %in% names(dataframe)){
    dataframe <- mutate(dataframe, converted = FALSE)
  }
  ...
  for (name in names){
    dataframe <- dataframe %>%
      dplyr::mutate("{name}" := ifelse(.data[[name]] > upper_limit |
                    ...
                    converted := ifelse(is.na(converted), FALSE, converted)) %>%
      dplyr::group_by(date) %>%
      dplyr::mutate("{name}" := ifelse(sum(converted)/n() > 0 & sum(converted)/n() < 1 & converted == 1, NA, .data[[{name}]]),
                    "{name}_std" := sd(.data[[name]], na.rm = TRUE))
  }
  ...
  # be cleaned
  if (rm.conflicted_duplicates){
    for (name in names){
      conflicted_data <- dataframe %>% dplyr::filter(.data[[paste0(name, "_std")]] >= 0.02)
    ...
    for (name in names){
      dataframe <- dataframe %>%
        dplyr::mutate("{name}" := ifelse(.data[[paste0(name, "_std")]] < 0.02 |
                                           is.na(.data[[paste0(name, "_std")]]), .data[[name]], NA))
      dataframe <- dataframe %>% dplyr::mutate("{name}" := non_NA(.data[[name]]))
      dataframe <- dataframe %>% dplyr::mutate("{name}" := ifelse(
        !is.na(.data[[paste0(name, "_std")]]) &
          .data[[paste0(name, "_std")]] > 0 & .data[[paste0(name, "_std")]] < 0.02,
        mean(.data[[{name}]]),
        .data[[{name}]])
      )
    ...
    for (name in names){
      dataframe <- dataframe %>% dplyr::mutate("{name}" := non_NA(.data[[name]]))
      dataframe <- dataframe %>% dplyr::mutate("{name}" := ifelse(!is.na(.data[[paste0(name, "_std")]]) &
                                                                    .data[[paste0(name, "_std")]] >= 0.02,
