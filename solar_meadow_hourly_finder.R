...
Colate_meadow_solar <- function(){
  # The directory that shall be recursively searched for snowdepth data
  file_dir = "/media/ross/external/Copying_files/archive/duplicateCSV/AF_database/AF_field_data_by_year/2001/Climat_data_2001/"
  # The directory in which all results shall be stored
  output_dir = "./meadow_solar/"
  ...
  while (!is.na(file_path)){
    ...
    if (grepl("cass|hobo|tower", file_path, ignore.case = TRUE)){
      file_path <- next_file(output_dir)
      next}
    ...
    is_identefier <- identifier_identifier(dataframe, target, is_day)
...
identifier_identifier <- function(dataframe, target, is_day){
  ...
  for (i in 1:length(dataframe)){
    if (sum(grepl(target, dataframe[[i]])) > 100 &
        !identical(dataframe[i], dataframe[is_day])){
...
solar_identifier <- function(dataframe, file_path, output_dir){
  ...
  dataframe <- dataframe %>% convert(num(names(dataframe[1:(length(dataframe) - 2)])))
  for (i in 1:length(dataframe)){
    ...
    if (length(dataframe_i[[i]]) == 0){
      ...
      next}
    squeezed_range <- unname(quantile(dataframe_i[[i]], na.rm = TRUE))[c(2,4)]
    if (is.na(squeezed_range[1]) |
        is.na(squeezed_range[2]) |
        squeezed_range[1] < 0 |
        squeezed_range[2] > 1) {
      is_solar <- append(is_solar, FALSE)
      next}
    winter_months <- dataframe_i %>% filter(month(datetime) %in% c(11, 12, 1))
    if(length(winter_months[[i]]) > 0){
      is_zero_ratio <- (winter_months[[i]] < 0.01)/length(winter_months[[i]])
      if(is_zero_ratio < 0.8){
        is_solar <- append(is_solar, FALSE)
        next}
    }
    summer_months <- dataframe_i %>% filter(!(month(datetime) %in% c(10, 11, 12, 1, 2, 3)))
    ...
    difficult_months <- dataframe_i %>% filter(month(datetime) %in% c(10,2,3))
...
construct_solar_data <- function(dataframe, is_day, is_year,is_hour,  is_identifier,
                                is_solar, file_path){
...
save_solar <- function (solar_dataframe, output_dir, is_year){
  if (length(is_year) > 0 && sum(is_year) > 0){ #If a year column has been identified
    years <- solar_dataframe[[4]] # Year will be the second column
  ...
  if (first_year != last_year){
    file_name <- paste0(output_dir, "/meadow_solar_",as.character(first_year),
